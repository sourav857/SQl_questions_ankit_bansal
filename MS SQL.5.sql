/*This is a very interesting SQL problem where we need to measure Success of a Marketing Campaign
Marketing Campaign Success: You have a table of in-app purchases by user.
Users that make their first in-app purchase are placed in a marketing campaign 
where they see call-to-actions for more in-app purchases. Find the number of 
users that made additional in-app purchases due to the success of the marketing campaign.
...
do we count users that over time purchase only the products they purchased on the first day.*/
...
(	
  select
  *,dense_rank() over(partition by user_id order by created_at) 
  as rn
